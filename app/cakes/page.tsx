// app/all-cakes/page.tsx  (or wherever AllCakes lives)
"use client";
...
import CakeGallerySkeleton from "../skeltonLoading/CakeGallerySkeleton";
...
const AllCakes = () => {
  ...
  // Fetch cakes + categories in parallel
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [cakesRes, categoryRes] = await Promise.all([
          axios.get(`${api}/api/cakes/find/cakes`),
          axios.get(`${api}/api/category/get/category`),
        ]);
        setAllCakes(cakesRes.data);
        setFilteredCakes(cakesRes.data);
  ...
  if (loading) return <CakeGallerySkeleton />;
      ...
      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 mt-6 sm:mt-8">
        <div className="text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-serif font-black text-stone-800 tracking-tight">
            The Cake{" "}
            <span className="font-light italic text-pink-600/90">Gallery</span>
          </h2>
